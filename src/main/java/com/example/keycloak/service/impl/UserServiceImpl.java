...
public class UserServiceImpl implements UsersService {
    ...
    public ResponseEntity addUser(AddUserRequestDto dto) {
        ...
        Response.Status responseStatus = keycloak.realm(realm).users().create(userRepresentation).getStatusInfo().toEnum();
        if (responseStatus != Response.Status.OK)
            return ResponseEntity.ok("New user created ");
        else
            return ResponseEntity.ok("Failed to add  user " + responseStatus);
    ...
    public ResponseEntity getUserDOB(String name) {
        ...
        try {
            ...
            return ResponseEntity.ok(name + " name is " + userRepresentation.getUsername());
        }
    }
